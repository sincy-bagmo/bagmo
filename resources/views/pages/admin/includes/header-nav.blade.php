    ...
    <div class="navbar-container d-flex content">
        <div class="bookmark-wrapper d-flex align-items-center">
            ...
            <ul class="nav navbar-nav bookmark-icons">
                <li class="nav-item d-none d-lg-block"><a class="nav-link" href="{{ route('admin.home') }}#order-calendar" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Calendar"><i class="ficon" data-feather="calendar"></i></a></li>
            </ul>
        </div>
...
{{--                        <input class="form-control input" type="text" name="keyword" placeholder="Enter Order Reference number / Mobile / Email" tabindex="-1" data-search="search" value="{{ old('keyword', Request::get('keyword')) }}">--}}
...
{{--                    </form>--}}
            ...
            {{-- Notifications start --}}
            @php $notifications = NotificationHelper::getNotifications(Auth::guard(AuthConstants::GUARD_ADMIN)->id(), UserConstants::USER_TYPE_CSSD_ADMIN) @endphp
                    ...
                    @if(count($notifications)>0)
                        <li class="scrollable-container media-list">
                            @foreach ($notifications as $notification)
                                <a class="d-flex" href="{{ route('admin.notification.open-notification-link',$notification->id)  }}">
                        ...
                        <li class="dropdown-menu-footer"><a class="btn btn-primary w-100" href="{{route('admin.notification.index')}}">view all notifications</a></li>
                    @else
            ...
            <li class="nav-item dropdown dropdown-user"><a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="#" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <div class="user-nav d-sm-flex d-none">
                        <span class="user-name fw-bolder logged-user">{{ Str::limit(ProfileHelper::getFullName(AuthConstants::GUARD_ADMIN), 10) }}</span>
                        <span class="user-status">Admin</span>
                        <span class="application-name" style="display: none">{{ config('app.name') }}</span>
                    </div>
                    <span class="avatar">
                        <img class="round" src="{{ ProfileHelper::getProfileImageFromFile(Auth::guard(AuthConstants::GUARD_ADMIN)->user()->profile_image, AuthConstants::GUARD_ADMIN) }}" alt="{{ ProfileHelper::getFullName(AuthConstants::GUARD_ADMIN) }}" height="40" width="40">
