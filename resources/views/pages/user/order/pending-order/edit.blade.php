...
<section class="content">
    <div class="container-fluid">
        <form method="POST" action="{{ route('user.order.pending-order.update', $pending_order->id) }}">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-7 col-12">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Edit order request</h4>
                        </div>
                        <div class="card-body">
                            <div class="mb-1 row">
                                <div class="col-md-12  col-12">
                                    <div class="mb-1">
                                        ...
                                        <select name="operation_id" id="operation_id" class="form-select">
                                            ...
                                            <option value="{{ $key }}" {{ (old('operation_id', $pending_order->operation_id) == $key  ) ? 'selected' : '' }}>{{ $item }}</option>
                                            @endforeach
                                        </select>
                                        @error('operation_id')
                            ...
                            <div class="mb-1 row">
                                <div class="col-md-12  col-12">
                                    <div class="mb-1">
                                        ...
                                        <input type="text" class="form-control @error('doctor_name')is-invalid @enderror" id="doctor_name" name="doctor_name" value="{{ old('doctor_name', $pending_order->doctor_name) }}" placeholder="Enter Doctor Name" @error('doctor_name')aria-describedby="doctor_name-error" aria-invalid="true" @enderror>
                                        @error('doctor_name')
                            ...
                            <div class="mb-1 row">
                                <div class="col-md-12  col-12">
                                    <div class="mb-1">
                                        ...
                                        <input type="text" class="form-control @error('booking_date')is-invalid @enderror" id="booking_date" name="booking_date" placeholder="Select required date" value="{{ old('booking_date', $pending_order->booking_date) }}" @error('booking_date')aria-describedby="booking_date-error" aria-invalid="true" @enderror autocomplete="off" required>
                                        @error('booking_date')
                            ...
                            <div class="col-md-12  col-12">
                                <div class="mb-1">
                                    ...
                                    <input type="text" class="form-control @error('description')is-invalid @enderror" id="description" name="description" value="{{ old('description', $pending_order->description) }}" placeholder="Enter description" @error('description')aria-describedby="id_card_number-error" aria-invalid="true" @enderror>
                                    @error('description')
                                ...
                                <div class="col-md-12  col-12">
                                    <div class="mb-1">
                                        ...
                                        <select name="tray_category_id[]" id="tray_category_id" class="form-select select2" multiple="multiple">
                                            ...
                                            <option value="{{ $key }}" {{ (old('tray_category_id', $pending_order->tray_category_id) == $key  )  ? 'selected' : '' }}>{{ $item }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-12  col-12">
                                    <div class="mb-1">
                                        ...
                                        <select name="instrument_category_id[]" id="instrument_category_id" class="form-select select2" multiple="multiple">
                                            ...
                                            <option value="{{ $key }}" {{ (in_array($key, $additionalInstrumentsLinked)) ? 'selected' : '' }}>{{ $item }}</option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="card-body">
                                    <div class="table-responsive">
                                        <table class="table">
                                            ...
                                            <tbody class="additional-instruments">
                                                @foreach($additionalInstruments as $item)
                                                <tr id="remove-instrument-{{ $item->id }}">
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            <div class="fw-bolder" >{{  Str::limit($item->category_name, 20) }}</div>
                                                        </div>
                                                    </td>
                                                    <td>
                                                        <div class="d-flex align-items-center">
                                                            <div class="d-flex flex-column">{{ $item->code }}</div>
                                                        </div>
                                                    </td>
                                                    <td><div class="input-group"><input class="touchspin-cart" type="number" name="category_id_{{ $item->id }}" value="{{ old('qty', $item->total) }}"></div></td>
                                                </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
...
<script>
    var app = {
        initialize: function() {
            ...
            $(window).on("load", function() {
                ...
                $('#operation_id').on('change', function() {
                    ...
                    url = url + '/order/operation-details/' + $(this).val();
                    app.addOperationTraysToCart(url);
                });
                var orderId = '{{ $pending_order->id }}';
                var url = '{{ route('user.home') }}';
                url = url + '/order/operation-details/' + operationId;
                app.editOperationTraysToCart(url);


                var url = '{{ route('user.home') }}';
                url = url + '/order/' + orderId +'/additional-trays-details/' + operationId;
                app.editAdditionalTraysToCart(url);
            });
        ...
        addOperationTraysToCart: function(url) {
            $.ajax({
                url: url,
                method: 'GET',
        ...
        editOperationTraysToCart: function(url) {
            $.ajax({
                url: url,
                method: 'GET',
                success: function(response) {
        ...
        editAdditionalTraysToCart: function(url) {
            $.ajax({
                url: url,
                method: 'GET',
                success: function(response) {
